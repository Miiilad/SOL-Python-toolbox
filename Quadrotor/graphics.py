...
import numpy as np
class Sim3D():
    def __init__(self,episode,Database,x_init,len_t,u_lim,x_ref,measure_dim,realtime):
        self.pad=0.14
        self.lim_bottom=-1
        ...
        self.DB=Database
        scene.camera.pos=vector(0.6,3,2.7)
        self.realtime=realtime
        self.ref=vector(x_ref[0],x_ref[2],x_ref[1])
        self.x_ref=x_ref
        self.x_init=x_init
        ...
        self.quad_init_position=vector(x_init[0],x_init[2],x_init[1])
        #base_red=box(pos=quad_init_position,length=0.3,height=0.01,width=0.04,color=color.black)
        self.base_red=sphere(pos=self.quad_init_position, color=color.black, length=0.35,height=0.02,width=0.05)
        self.base_red.rotate(angle=np.pi/4, axis=vector(0,1,0),origin=self.quad_init_position)
        ...
        self.base_green=sphere(pos=self.quad_init_position, color=color.black, length=0.05,height=0.011,width=0.35)
        self.base_green.rotate(angle=np.pi/4, axis=vector(0,1,0),origin=self.quad_init_position)
        self.tip_x=cylinder(pos=self.quad_init_position,axis=vector(0.4,0,0),radius=0.002,color=color.red,opacity=0.5)
        self.tip_y=cylinder(pos=self.quad_init_position,axis=vector(0,0.4,0),radius=0.002,color=color.blue,opacity=0.5)
        self.tip_z=cylinder(pos=self.quad_init_position,axis=vector(0,0,0.4),radius=0.002,color=color.green,opacity=0.5)
        prop1_pos=vector(0.1,0.02,0.1)+self.quad_init_position
        prop3_pos=vector(-0.1,0.02,-0.1)+self.quad_init_position
        prop2_pos=vector(-0.1,0.02,0.1)+self.quad_init_position
        prop4_pos=vector(0.1,0.02,-0.1)+self.quad_init_position
        ...
        self.core = extrusion(pos=self.quad_init_position,path=paths.cross(width=0.08, thickness=0.025),shape=shapes.circle(radius=0.015),texture="https://i.imgur.com/zn5QlC8.jpg")
        scene.camera.axis=self.base_red.pos-scene.camera.pos
        self.x_init=self.x_init-x_ref
    def update_realtime(self,i,sample,u,pqr,h):
        p,q,r=pqr
        x_dif=sample-self.x_ref-self.x_init
        self.x_init=np.array(sample-self.x_ref)
        if self.realtime:
            ...
            # self.rotate(radians(x_dif[9]),vector(1,0,0))
            # self.rotate(radians(x_dif[11]),vector(0,1,0))
            # self.rotate(radians(x_dif[10]),vector(0,0,1))
            #print(sol.y[...,-1])
    ...
    def run(self,i,h):
        if not self.realtime:
            for ii in arange(i):
                ...
                self.ring2.rotate(angle=0.02, axis=vector(0,1,0),origin=self.ref)
                error=self.base_red.pos-self.ref
                if np.linalg.norm([error.x,error.y,error.z])<0.16:
                    self.ring2.color=color.yellow
                    self.ring1.color=color.yellow
                else:
                    self.ring2.color=color.white
                ...
                rate(50)
                self.prop_rot(u)
                #print('pos',self.base_red.pos)
                self.rotate(p*h,self.tip_x.axis)
                self.rotate(r*h,self.tip_y.axis)
                self.rotate(q*h,self.tip_z.axis)
                # self.rotate(radians(x_dif[9]),vector(1,0,0))
                # self.rotate(radians(x_dif[11]),vector(0,1,0))
                # self.rotate(radians(x_dif[10]),vector(0,0,1))
                self.move(vector(x_dif[0],x_dif[2],x_dif[1]))
