    ...
    def update(self):
        if self.ID_alg['SINDy']:
            if self.DB.db_overflow:
                self.Weights=(utils.SINDy(self.DB.db_X_dot,self.DB.db_Theta,lam=1e-5))
            else:
                self.Weights=(utils.SINDy(self.DB.db_X_dot[:,:self.DB.db_index],self.DB.db_Theta[:,:self.DB.db_index],lam=1e-5))
            return self.Weights
